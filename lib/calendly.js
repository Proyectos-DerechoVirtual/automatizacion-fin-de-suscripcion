...
export async function findPhone(email) {
  try {
    ...
    // Buscar teléfono en Q&A o en text_reminder_number
    const phoneKeywords = ['teléfono', 'telefono', 'tel\u00c3\u00a9fono', 'phone', 'whatsapp', 'móvil', 'movil', 'celular', 'numero'];

    for (const inv of invitees) {
      // 1. Buscar en questions_and_answers
      ...
      // 2. Fallback: usar text_reminder_number
      if (inv.text_reminder_number) {
        let phone = inv.text_reminder_number.replace(/[^0-9+]/g, '');
        if (phone && !phone.startsWith('+')) {
          phone = phone.length <= 9 ? `+34${phone}` : `+${phone}`;
        }
        if (phone) return { phone, calendlyFound: true };
